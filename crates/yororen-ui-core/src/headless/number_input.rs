...
impl NumberInputProps {
    ...
    ///
    /// One-way data flow: renderer's `compose` owns the entire
    /// pipeline (state, wrapper, inner element, keymap, the
    /// +/- stepper handlers, hover / active). Headless just
    /// forwards the call.
    pub fn render(self, cx: &mut gpui::App, window: &mut gpui::Window) -> gpui::AnyElement {
        ...
        use crate::renderer::number_input::NumberInputRenderer;
